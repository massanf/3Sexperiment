...
void pad(complex double *X, int start, int end){
...
int main(int argc, char ** argv) {
  ...
  complex double * newY = calloc(sizeof(complex double), n);
  ...
  */
  ...
  while (1) {
    ...
    /*距離による減衰*/
    for(int i = 0; i < n; i++){
        newY[i] = newY[i] * volume / exp(gensui*log10(fabs(sqrt(x*x+d*d)))); //100と4は適当なのでいじって (100は音量、4は減衰の具合)
    }
